...
- `2026-08-11_2245_one-turn-grounded-match-v0.md`：实现短期规划最小单元：一个数学目标对当前全部合法放骰行为逐项做一次真实状态转移，输出完成/部分/无关/有害/无效；6组测试和Roswell真实母舰触发/AA避险用例通过，尚未进入多目标、递归和五骰整轮组合。
- `2026-08-11_1634_acquisition-meaning-geometry-v0.md`：隔离实现普通概念与`获得(概念)`面；科技直接获得胜利意义，能源只在研究房缺电后进入二层意义链，建设/挖掘只在最终房间锁定后进入；768维几何和组合卡点通过，尚缺渐进关系学习。
- `2026-08-11_1547_ufs-incremental-rule-cognition.md`：按规则书页面顺序建立8阶段追加式认知；第9页冻结为首局边界，得到33概念、21条规则知识和11个行为，机器人/战役/内部字段泄漏为0；同时确认旧零规划符文仍含人工策略、未兑现房间收益和超人枚举。
- `2026-08-11_1453_roswell-zero-plan-rune-baseline.md`：在正式 Roswell 地图上实现纯零规划 768 维符文玩家；每步只做当前需求与即时选项坐标点积。30 个相同种子下研究、挖掘和存活明显优于随机，但双方均 0 胜，明确了局部语义匹配不能替代跨步骤计划。
- `2026-08-11_1344_roswell-threat-0-complete-map.md`：合并 Roswell 城市参数、A+B、四块天空简单面和从下到上的16格研究费用，生成首张完整正式地图；开局68个合法落点，100局随机冒烟0卡死，两类失败链正常触发。
- `2026-08-11_1327_roswell-threat-0-sky-ingested.md`：校验并冻结用户录入的四块天空简单面；16 行80格、19爆炸、15箭头、8母舰下降与8侧轨动作通过规则合同，真实基地＋天空已可执行，尚缺真实城市／研究值。
- `2026-08-11_1306_ufs-sky-fixed-four-rows.md`：更正 1223 报告的可变行数设计；每块天空板固定 4 行、四块共 16 行，移除增删行按钮，并为早期三行本地存档自动补第四行；专项测试与 13 项规则回归通过。
- `2026-08-11_1223_ufs-sky-city-map-editor.md`：新增天空＋城市统一录入器；四块天空板支持换序、简单／威胁面、五列图标、右侧多重母舰效果与骷髅行，Roswell 可录伤害、能源和研究轨道；4 项模型测试、静态检查及原引擎 13 项回归通过。
- `2026-08-11_1140_roswell-base-route-direction-correction.md`：更正 0056 报告的路线方向；完整 30 格通道从 A 右上开始、到 B 右下结束，开局挖掘机在 A 第三排右侧，未来 20 格路径到 B 右下，专项测试和 SVG 复核通过。
- `2026-08-11_0056_roswell-base-ab-route.md`：把用户录入的 A+B 30 格、25 房间转成正式 fixture，补全从 A 右下到 B 右下的 20 格蛇形挖掘路径与逐格解锁；官方规则确认纯通道格可放骰但无房间效果，专项测试与 SVG 复原图通过。

## 2026-08-10

- `2026-08-10_2344_ufs-base-map-editor.md`：新增独立的基地 A/B 五列可视化录入器，支持多格房、修正、能耗、解锁深度与挖掘路径录入，并导出可继续编辑的局部地图 JSON；原有 13 项正式规则测试保持通过。
...
- `2026-08-04_0004_player-value-landscape-v0.md`：隔离实现玩家价值地形：R只在完整队伍附近形成局部强弱锚点，EVerify才将价值推广到确认概念；护盾环境隔离、全局贬值、冲突证据信心和坐标零漂移9/9通过。50队隐藏验证总体Spearman 0.5105、六环境Top-10均提升；火焰爆发失败由隐藏真值oracle定位为上游队伍坐标缺少组合机制语义。

## 2026-08-02

- `2026-08-02_0047_semantic-team-coordinate-v0.md`：隔离验证队伍认知从768维零点被自然语言知识逐条拉动、需求句直接点积检索；50队完整知识对第二种子存活相关0.757，移除胜负结果后仍为0.394，治疗保护0.832。重复饱和与正负反转通过；控制和启动因现有知识只有技能名、缺少机制语义而失败，下一步应修知识形成层。

## 2026-08-01

- `2026-08-01_1808_llm-generated-team-directions-v2.md`：独立无上文LLM仅凭玩家可见知识，为8个失败场景生成100点有限预算的九维需求方向，再与50队知识向量点积选队；选择不再塌缩为同一综合强队，8/8方向得分优于原队及池平均、Top-5胜率8/8优于全池、Top-1翻胜6/8。尚需未见敌队和多次独立采样验证。

- `2026-08-01_1741_team-vector-guess-v1.md`：50支允许重复角色的有序队伍对六固定流派跑两套随机种子共600场；玩家可见signal形成九维队伍知识向量，原子需求形成方向后点积选队。九轴跨种子平均Spearman 0.979，七类需求Top-10在未参与学习的随机种子中全部优于Bottom-10，最多8条明确方向线索后猜队伍50/50 Top-1；尚未验证未见敌队。

- `2026-08-01_1551_learned-role-guess-v1.md`：从冻结22场玩家可见信号导出13个角色的32条粗能力与16条细特点知识；GTE对模糊问题的正确方向Top-3召回20/21，最多6条相对线索后13个目标全部留在Top-3，未知治疗信息未被当零。当前只证明玩家认知内部找人链路，尚未用未见战斗验证真实效用。

- `2026-08-01_0127_gte-latent-space-rune-v0.md`：隔离下载并离线运行GTE多语言基础向量模型，建立40角色、九语义轴的高维符文猜测。真值坐标40/40命中，GTE九轴平均AUC 0.846但多轮Top-1仅10%，确认方向存在但简单概率连乘不可用；正式玩家Agent未修改。
