...
`game_data/causal-chain-event-matcher.js` is the isolated structured matcher in front of that resolver. Agent steps use an allowlisted predicate/ref/qualifier/environment DSL; natural-language statements are display-only. The matcher reads only public semantic event IDs, stable player character references or player-visible concept/entity references, frozen information tiers, and event time. It rejects raw `left-*`/`right-*` identities, hidden fields, custom predicates, Agent support/strength, and observational claims of `primary_cause`. The battle parser now emits a parallel `causalEvidence` channel from already player-visible raw events. Each kill, damage, skill, shield, supported control/buff, ally death, and combat result is independently passed through the same low/ordinary/high perception model and projected to opaque public references. This channel is stored separately by the received-information organizer, is never routed to ordinary observations or causal knowledge, and is available only for current-hypothesis verification. The aggregate `enemy_defeated` sentence remains unchanged for ordinary summarization. Sixteen deterministic matcher cases now include a real fixture in which perceived ranger damage, the ranger's exact target kill, and the later win confirm a three-step path.
