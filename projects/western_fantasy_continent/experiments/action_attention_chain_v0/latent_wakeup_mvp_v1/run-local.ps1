...
& $Python (Join-Path $ExperimentRoot "test_contract.py")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $Python (Join-Path $ExperimentRoot "run_mvp.py")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $Python (Join-Path $ExperimentRoot "run_holdout.py")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $Python (Join-Path $ExperimentRoot "test_matrix_equivalence.py")
exit $LASTEXITCODE
