# 五槽轨迹记忆 V0

这是一个隔离、可复用的长期经历模块。它不保存“放骰子后飞机移动”这类动作字符串作为检索条件，而把玩家感知到的一件变化拆成五个连续语义槽位：

```text
q = [
  受影响对象向量,
  变化趋势向量,
  原因关系向量,
  时间状态向量,
  上下文向量
]
```

五个槽位分别编码后按权重拼接。若底层GTE为768维，一个`q`就是3840维。记忆保存的是`q → q_next`；运行时把所有已记住的`q`编译成矩阵，一次矩阵乘法唤醒相似经历，再由相近后继共同支持下一坐标。

## 初始化新的空记忆

```python
from five_slot_memory import FiveSlotCoordinate, FiveSlotTrajectoryMemory
from gte_encoder import LocalGTEEncoder

encoder = LocalGTEEncoder()
memory = FiveSlotTrajectoryMemory.new(encoder)
assert memory.is_empty
```

不同实例完全独立。新游戏、新玩家或隔离实验都可以各自`new()`一个。

## 假设每一步都已注意：抽象写入

在暂时不研究注意力漏失时，`noticed_step_writer.py`接收一个已经看清的步骤：主体做了什么、作用于谁、对象前后如何变化、何时发生以及处于什么场景。它不猜测任何缺失内容，只把完整事实机械整理成五槽：

```python
from noticed_step_writer import FullyNoticedStep, FullyNoticedTrajectoryWriter

steps = [
    FullyNoticedStep(
        actor="玩家",
        action="把骰子放入第三列",
        affected_object="第三列上方的飞船",
        before_state="飞船停在较高行",
        after_state="飞船开始向下移动",
        temporal_state="动作已经发生并被完整看见",
        context="真实UFS对局",
    ),
    FullyNoticedStep(
        actor="向下移动的飞船",
        action="到达箭头格",
        affected_object="同一艘飞船",
        before_state="飞船位于箭头上方",
        after_state="飞船沿箭头横向移动",
        temporal_state="后续步骤已经发生并被完整看见",
        context="真实UFS对局",
    ),
]

FullyNoticedTrajectoryWriter.remember(memory, steps)
```

这层的输入合同就是“玩家已经注意到每一步”。如果对象或状态缺失，它会报错而不会自行补写。注意力层以后只需负责产生这些完整步骤；记忆层不需要知道玩家是怎样看到它们的。

## 记住一条连接

```python
q_now = FiveSlotCoordinate(
    affected_object="可爆炸装置",
    change_trend="从完整安静变为被移动物体直接碰撞",
    cause_relation="运动物体与装置发生物理接触",
    temporal_state="碰撞正在发生，结果尚未结算",
    context="真实游戏局面的短期预想",
)

q_next = FiveSlotCoordinate(
    affected_object="同一个可爆炸装置",
    change_trend="从受到碰撞变为已经爆炸",
    cause_relation="碰撞触发内部爆炸机制",
    temporal_state="后果已经发生",
    context="真实游戏局面的后果",
)

memory.remember(q_now, q_next, strength=1.0)
result = memory.query(q_now)
print(result.following)
```

同一条精确连接再次`remember()`不会复制矩阵行，而会增加`observations`和`support`。多条措辞不同但坐标相近的经历则保留为不同记录，在查询时共同支持相近后继。

一整段轨迹可以直接加入：

```python
memory.remember_trajectory([q1, q2, q3, q4])
```

这会生成`q1→q2`、`q2→q3`、`q3→q4`三条可被重新组合的连接。

## 保存和恢复以前的记忆

```python
memory.save("stores/player_a.json", include_cache=True)

restored = FiveSlotTrajectoryMemory.load(
    "stores/player_a.json",
    encoder,
)
```

保存文件只包含五槽文本、槽位权重、连接强度和可选元数据，不固化模型向量。加载后使用当前传入的编码器重新编译矩阵，因此同一份经历可以在以后更换编码器时重建；正式比较不同模型时应记录编码器版本。

当`include_cache=True`时，会在JSON旁生成一个可丢弃的`.vectors.npz`缓存。编码器身份一致时，`load()`直接恢复起点矩阵、后继矩阵和粗筛索引；换编码器或删除缓存后仍会从JSON原始五槽重新构建。

批量查询先用五槽在同一语义空间中的加权合成坐标粗筛默认128条候选，再对候选计算完整3840维五槽相似度。粗筛只优化速度，最终分数仍来自五槽拼接坐标。

## 边界

- 该模块只负责“当前五槽坐标唤醒哪段后续”，不负责从画面或自然语言自动抽取五槽。
- `following`是记忆中最接近聚合后继中心的一条可读坐标；`prediction_vector`才是多条后继加权后的连续坐标。
- 相似度、置信度和默认槽位权重尚未接玩家感知幅度，当前仅用于隔离验证。
- 轨迹记忆是联想候选，不替代游戏规则的真实结算。

## 运行验证

```powershell
.\run-local.ps1
```

验证包含：全新空实例、实例隔离、精确连接、重复经历增强、连续轨迹、保存/加载，以及真实GTE对“真实碰撞、规则假设、只观察未接触”三种语义的区分。

`run_bulk_memory_validation.py`还会读取此前冻结的5000条学习经历和5000条独立评测经历，把已有主体槽位、互动和状态结果机械转换成五槽坐标，验证：

- 5000条`q→q_next`能否编译成矩阵；
- 500条学习经历抽样重放能否取回自己保存的后继；
- 5000条记忆相对固定前100条记忆，能否提高独立查询的同因果后继召回；
- 5000条实例保存、重新加载、重新编码后，抽样预测是否保持一致；
- 实际矩阵内存与批量查询时间。

这个大量测试使用的是此前由共同因果家族生成的结构化数据，因此用于验证容量、矩阵检索和持久化，不作为“自主发现新因果家族”的证据。

大量测试较慢，和快速验证分开运行：

```powershell
.\run-bulk.ps1
```

当前一次完整结果：5000个不同五槽坐标成功建库；500条原经历重放500/500；1000条独立查询中的900条已见因果召回855条（95%），固定只用前100条记忆的对照为54.67%；缓存后1000条查询1.60秒。冷构建5000条约431秒，坐标起点+后继矩阵153.6MB，缓存文件约169.3MB。对未学习因果的留出拒绝率只有23.75%，因此本模块只能唤醒经历，不能替代未知规则识别。
