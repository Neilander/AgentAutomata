...
- 选择格输出 `choice`。
- 陌生格输出 `unknown`。
- 轨迹程序实际需要读取的事实未被注意时，不泄漏该事实并输出 `attention_stop`。
- 已经完成移动、但下一落点类型未被注意时，保留已想到的移动并以`unnoticed_endpoint_effect_omitted_from_imagination`结束这条粘连；这表示AI可能漏掉箭头、母舰格等附加效果并带着错误脑内状态继续，而不是撤销整个动作。
