...
for (let i = 0; i < lines.length; i += 1) {
  ...
  const observedEnergy = Number(publicResponse.observation?.energy);
  if (Number.isFinite(observedEnergy) && observedEnergy < 0) {
    throw new Error(`negative energy at ${lines[i].sequence}`);
  }
...
const validEarlyTerminal = last.status === "complete";
if (!(validStagePause || validEarlyTerminal)) {
