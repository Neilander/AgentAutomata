...
const dir = __dirname;
const lines = fs.readFileSync(path.join(dir, "machine-records.ndjson"), "utf8")
  .trim().split(/\r?\n/).filter(Boolean).map(JSON.parse);
if (!lines.length || lines[0].command !== "start" || lines.filter((row) => row.command === "start").length !== 1) {
...
for (let index = 0; index < lines.length; index += 1) {
  ...
  if (!row.public || !Array.isArray(row.public.availableOperations)) {
    throw new Error(`missing public response at ${row.sequence}`);
  }
  ...
  const prior = lines[index - 1]?.public;
  if (row.command === "advance") {
    const payload = JSON.parse(fs.readFileSync(path.join(dir, row.payloadFile), "utf8"));
    if (!prior?.availableOperations?.includes(payload.type)) {
      throw new Error(`operation not publicly offered at ${row.sequence}`);
    }
  }
  if (row.command === "random" && prior?.status !== "random") {
    throw new Error(`random outside boundary at ${row.sequence}`);
  }
}
