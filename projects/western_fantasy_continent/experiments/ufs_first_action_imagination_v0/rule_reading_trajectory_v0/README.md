...
- `source_rules.json`：从既有分段规则认知中冻结的规则句，以及现场试玩后由公开地图补出的通道无产出事实。
- `ai_compiled_trajectories.json`：26条严格 `q当前 → q后续`。
- `compiled-trajectory-loader.js`：JS侧校验来源、严格五槽、重复ID和禁止补数字；当前只把与第一步有关的6条轨迹装入放置实验。
- `compile_gte_matrix.py`：复用既有 `RuleTrajectoryCompiler` 做正式结构编译，再用本地真实GTE构建激活矩阵。
- `artifacts/initial_rule_memory.json`：可恢复的26条五槽轨迹记忆。
...
26条轨迹覆盖：目标与胜负、普通同列下降、防空修正、白骰重投、挖掘放置与执行、多格/单格房、通道无普通产出、箭头、母舰下降格、撞城、能源成本、能源/战斗机/研究房、研究顺序、母舰阶段、行行动、飞船生成优先级和最终研究门槛。
...
- D（现场补边回归）：灰2放入普通通道；唤醒普通下降与通道无产出轨迹，继续到下一选择而非Unknown。
...
- 目前真正进入一步运行的是6条放置相关轨迹；其余20条已生成、正式编译和GTE激活，但仍需在对应阶段的真实场景中逐条接线。
- 这26条仍不声称覆盖所有隐含组合、例外或玩家后来从反馈学到的轨迹。
