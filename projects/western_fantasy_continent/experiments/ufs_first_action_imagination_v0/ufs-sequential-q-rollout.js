...
function runSequentialRollout({ initialQ, steps, imagineStep, evaluateAnchor = evaluateEntityAnchor } = {}) {
  ...
  for (let index = 0; index < steps.length; index += 1) {
    ...
    const anchor = step.anchor == null
      ...
      : evaluateAnchor(qBefore, step.anchor, step);
    ...
    const imagined = imagineStep({ index, step: clone(step), qBefore: clone(qBefore) });
    ...
    if (imagined.stop) {
      if (typeof imagined.stop.status !== "string" || typeof imagined.stop.reason !== "string") {
        throw new TypeError("imagineStep stop must provide status and reason");
      }
      return {
        schema: "ufs_sequential_q_rollout_v1",
        status: imagined.stop.status,
        stoppedBeforeStep: index + 1 < steps.length ? index + 1 : null,
        stoppedAfterStep: index,
        stopReason: imagined.stop.reason,
        boundary: clone(imagined.stop.boundary || null),
        trace,
...
function revalidateNextStep({ actualQ, step, evaluateAnchor = evaluateEntityAnchor } = {}) {
  ...
  const anchor = step.anchor == null
    ? { status: "supported", reason: "no_anchor_required", evidence: null }
    : evaluateAnchor(actualQ, step.anchor, step);
  return {
