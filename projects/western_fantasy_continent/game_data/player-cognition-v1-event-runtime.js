...
function createState(seed = "event-player", configInput = {}) {
  ...
  return {
    schema: "player_cognition_event_state_v1",
    seed,
    config,
    emotion: {
...
function collectHypothesisEvidence(state, event) {
  ...
  for (const row of rows) {
    if (event.result.kind !== "action_summary" && !outcomeKinds.has(row.resultKind)) continue;
...
function applyFailureLearning(state, event, pattern) {
  ...
  if (!memory) {
    memory = { key, failures: 0, fear: 0, preferHigherSuccessActions: false };
