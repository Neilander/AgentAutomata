...
function selectNextAction(cognitionState, observation, options = {}) {
  const terminal = terminalDecision(cognitionState, observation);
  if (terminal) {
    return { action: null, candidates: [], decision: null, cognitionState, terminal: true, reason: terminal };
  }
  const candidates = (observation?.allowedActions || [])
    .map((action) => scoreAction(cognitionState, observation, action))
...
function scoreSwap(state, observation, action) {
  ...
  const awaitingExperiment = (state.affordanceExperiments || []).find((row) => row.status === "awaiting_combat");
  if (awaitingExperiment) {
    return {
      ...
      known: false,
      hypothesis: null,
      components: { experimentInProgress: -1 },
      knowledgeBasis: null,
      failureBasis: null,
    };
  }
  ...
  const visibleMember = visibleTeamMember(observation, slotIndex);
  const militiaReplacement = experiment && visibleMember?.kind === "militia" ? 0.12 : 0;
  const novelExperiment = experiment ? 0.68 : 0;
  const lowActionCost = experiment ? 0.05 : 0;
  const score = (knownTeamBehavior ? 0.12 : 0.04) + attributionFit + novelExperiment + militiaReplacement + lowActionCost;
  const hypothesis = experiment ? {
    id: `try-new-character:${heroId}`,
    problem: `${heroId} joined the visible roster but has not been used`,
    cause: visibleMember?.kind === "militia"
      ? `a newly visible full character can replace visible militia ${visibleMember.name}`
      : "a newly visible character creates an untested team option",
    resultKind: "team_changed",
    target: heroId,
  } : null;
  return {
    action,
    ...
    hypothesis,
    components: {
      discoveredTeamEffect: knownTeamBehavior ? 0.12 : 0.04,
      failureAttributionFit: attributionFit,
      novelExperiment,
      militiaReplacement,
      lowActionCost,
    },
...
function choiceModeForSelection(state, selected) {
  if (selected.kind === "swap" && selected.hypothesis) return "test_new_character_affordance";
...
function terminalDecision(state, observation) {
  const unresolvedExperiment = (state.affordanceExperiments || []).some((row) => ["available", "awaiting_combat"].includes(row.status));
  if (unresolvedExperiment) return null;
...
function visibleTeamMember(observation, slotIndex) {
  const names = String(observation?.team || "").split("、").map((name) => name.replace(/（.*?）/g, "").trim());
  const name = names[slotIndex];
  return (observation?.roster || []).find((row) => row.name === name) || null;
}
