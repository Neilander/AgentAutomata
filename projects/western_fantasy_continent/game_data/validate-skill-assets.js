...
function validateRoleKits() {
  for (const [roleId, role] of Object.entries(SKILL_ASSETS.roleKits || {})) {
    ...
    assert(Number.isFinite(role.moveSpeed) && role.moveSpeed > 0, `${roleId}.moveSpeed must be positive`);
    assert(Number.isFinite(role.range) && role.range > 0, `${roleId}.range must be positive`);
    if (role.supportRange !== undefined) assert(Number.isFinite(role.supportRange) && role.supportRange > 0, `${roleId}.supportRange must be positive`);
